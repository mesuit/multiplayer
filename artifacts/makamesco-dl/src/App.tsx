...
export default function App() {
  const [view, setView] = useState<'home' | 'input' | 'search'>('home');
  ...
  const getDownloadData = async (type: string, param: string) => {
      ...
      {view === 'home' ? (
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-3 gap-6">
          {platforms.map(p => (
            <button
              key={p.id}
              onClick={() => { setPlatform(p); setView('input'); }}
        ...
        <div className="max-w-xl mx-auto bg-[#0f1420] border border-slate-800/50 p-8 rounded-[3rem] shadow-2xl">
          <button
            onClick={() => { setView('home'); setData(null); setInput(''); setSearchResults([]); }}
          ...
          {data && (
            <div className="mt-8">
              <div className="bg-[#161d2f] rounded-3xl overflow-hidden border border-slate-700 shadow-2xl">
                <img
                  src={data.thumbnail || data.thumb || 'https://via.placeholder.com/500x300?text=Media+Found'}
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-col gap-2">
                    <a
                      href={data.url || data.nowm || data.link}
                      target="_blank"
                      rel="noreferrer"
                      className="w-full bg-cyan-500 text-black py-4 rounded-xl text-center font-black uppercase text-xs hover:scale-[1.02] transition-transform"
                    >
                      Download HD Video
                    </a>
                    {(data.mp3 || data.audio) && (
                      <a
                        href={data.mp3 || data.audio}
                        target="_blank"
                        rel="noreferrer"
                        className="w-full bg-slate-800 text-white py-4 rounded-xl text-center font-black uppercase text-xs hover:bg-slate-700 transition-colors"
                      >
                        Download MP3 Audio
                      </a>
                    )}
          ...
          {view === 'search' && searchResults.length > 0 && (
            <div className="mt-8 space-y-3 overflow-y-auto max-h-[450px] pr-2">
              {searchResults.map((video, i) => (
                <div
                  key={i}
                  className="flex gap-4 p-3 bg-[#161d2f] rounded-2xl items-center border border-slate-700/50 hover:border-cyan-500/30 transition-all group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={() => getDownloadData('youtube', video.url || '')}
                        className="text-[10px] bg-cyan-500 text-black font-black px-3 py-1.5 rounded-md uppercase"
                      >
                        Video
                      </button>
                      <button
                        onClick={() => getDownloadData('youtube', video.url || '')}
                        className="text-[10px] bg-white text-black font-black px-3 py-1.5 rounded-md uppercase"
                      >
                        Audio
                      </button>
                    </div>
